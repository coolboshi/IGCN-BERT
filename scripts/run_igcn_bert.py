...
from models.igcn_bert.igcn_bert import IGCNBERT
...
def train_model(args):
    ...
    logger.info(f"l_layers: {config['l_layers']}")
    logger.info(f"d_A: {config['d_A']}")
    logger.info(f"M_igcn: {config['M_igcn']}")
...
def main():
    parser = argparse.ArgumentParser(description="IGCN-BERT Training and Testing")
    parser.add_argument("--mode", type=str, default="train", choices=["train", "test"], help="Mode: train or test (default: train)")
    parser.add_argument("--model", type=str, default="IGCN-BERT", help="Model name")
