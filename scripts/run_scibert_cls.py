...
from models.scibert_cls.scibert_cls import SciBERT_CLS
from train.trainer import Trainer
from utils.utils import setup_seed, setup_logger


def train_model(args):
    config_path = os.path.join("../configs", "scibert_cls", "config.json")
    with open(config_path, 'r', encoding='utf-8') as f:
        config = json.load(f)
    
    config["data_dir"] = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data")
    config["output_path"] = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "checkpoints", "scibert_cls")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SciBERT_CLS Training and Testing")
    subparsers = parser.add_subparsers(dest="command", help="Commands")
    
    train_parser = subparsers.add_parser("train", help="Train SciBERT_CLS model")
    ...
    test_parser = subparsers.add_parser("test", help="Test SciBERT_CLS model")
