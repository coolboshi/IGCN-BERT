...
from models.bert_mhan.bert_mhan import BERT_MHAN
...
def train_model(args):
    ...
    logger.info(f"Max sentences: {config['max_sentences']}")
    
    data_loader = DataLoader(config)
    train_loader, val_loader, test_loader = data_loader.get_dataloaders()
    
    model = BERT_MHAN(config)
...
def main():
    parser = argparse.ArgumentParser(description="BERT_MHAN Training and Testing")
    
    parser.add_argument("--mode", type=str, default="train", choices=["train", "test"], help="Mode: train or test (default: train)")
    parser.add_argument("--model", type=str, default="bert_mhan", help="Model name")
